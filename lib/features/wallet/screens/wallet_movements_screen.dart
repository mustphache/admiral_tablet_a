...
class _TotalsBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          _KV('Total', _fmt(total), v(true)),
          const Spacer(),
          _KV('In', _fmt(incoming), v()),
          const SizedBox(width: 16),
          _KV('Out', _fmt(outgoing), v()),
        ],
      ),
    );
  }
  ...
  static String _fmt(double x) =>
      x.toStringAsFixed(2).replaceAllMapped(RegExp(r'\B(?=(\d{3})+(?!\d))'), (m) => ',');
}

class _MovementTile extends StatelessWidget {
  final WalletMovementModel model;
  const _MovementTile({required this.model});
  ...
  IconData _iconFor(WalletType t) {
    switch (t) {
      ...
      case WalletType.refund:
        return Icons.reply_all; // دخل
      case WalletType.adjust:
        return Icons.tune; // سحب/تعديل
